...
from synthetic_data.immutable_release import (
    ...
    preserve_current_release_if_needed,
...
def write_pipeline_lock() -> Path:
    try:
        ...
        raise SyntheticVerificationError(
            f"Current synthetic release was not retained before lock update: {exc}"
        ) from exc
    reset_toolchain_identity_cache()
...
def write_release() -> dict[str, Any]:
    ...
    try:
        ...
        raise SyntheticVerificationError(
            f"Current synthetic release was not retained before rewrite: {exc}"
        ) from exc
...
def refresh_release() -> dict[str, Any]:
    ...
    try:
        ...
        raise SyntheticVerificationError(
            f"Current synthetic release was not retained before refresh: {exc}"
        ) from exc
    verify_pipeline_lock()
    _require_sources()
...
def _write_release_outputs(
    ...
    try:
        ...
        raise SyntheticVerificationError(
            f"Synthetic release could not be published immutably: {exc}"
        ) from exc
...
def check_release(*, recompile: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        raise SyntheticVerificationError(
            f"Current synthetic release is not retained immutably: {exc}"
        ) from exc
    return report
