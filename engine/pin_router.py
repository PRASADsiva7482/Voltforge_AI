...
class PinRouter:
    ...
    def resolve_i2c_connections(cls, component_name: str, board_type: str = "ARDUINO_UNO") -> List[Dict[str, str]]:
        ...
        vcc_pin = pinout.get("vcc_3v3") if "3" in pinout.get("vcc_3v3", "") else pinout.get("vcc_5v", "5V")
        return [
            {"from": f"{component_name}/VCC", "to": f"{board_type}/{vcc_pin}", "wireColor": "#ef4444", "reason": "Power rail"},
            {"from": f"{component_name}/GND", "to": f"{board_type}/{pinout.get('gnd', 'GND')}", "wireColor": "#111827", "reason": "Ground rail"},
...
if __name__ == "__main__":
    print("ESP32 I2C Pin Mapping for MPU6050:", PinRouter.resolve_i2c_connections("MPU6050", "ESP32"))
