"""V2 selected-file review: complete MIT grants qualify reserved copyright notices.

V1 acquisitions and their stricter exclusions remain immutable.
"""
import json
...
def review_license(source, license_bytes, raw, path):
    ...
    header = text.split("*/", 1)[0] if text.startswith("/*") else ""
    flatten = lambda value: " ".join(value.replace("*", " ").split())
    grant = flatten(license_text.removeprefix("MIT License"))
    explicit_mit = source["licenseId"] == "MIT" and grant in flatten(header) and "SPDX-License-Identifier: MIT" in header and "Copyright" in header
    review_text = text[len(header):] if explicit_mit else text
    conflicts = re.search(r"(?i)(all rights reserved|non[- ]commercial|no derivatives|SPDX-License-Identifier:\s*(?!MIT\b)[A-Za-z0-9.-]+|licensed under.*(?:GPL|Apache|BSD))", review_text)
    if conflicts: raise Quarantine("selected-file-license-exception-review-required")
    if source["licenseId"] == "MIT" and not explicit_mit: raise Quarantine("selected-file-MIT-notice-missing")
    return {"licenseId": source["licenseId"], "licenseSha256": sha(license_bytes), "selectedFileSha256": sha(raw),
            ...
            "trainingApproved": False, "noticesPreserved": True, "completeMITGrantChecked": explicit_mit}


def acquire(output_root=None, *, fetcher=fetch):
    output_root = Path(output_root or AI / "corpus/acquisition/v2").resolve()
...
def verify(path):
    ...
    expected = {(s["sourceId"], name) for s in CATALOG["sources"] for name in [s["licensePath"], *s["paths"]]}
    actual = [(r["sourceId"], r["upstreamPath"]) for r in manifest["files"]] + [(r["sourceId"], r["path"]) for r in manifest["denied"]]
    if len(actual) != len(set(actual)) or set(actual) != expected: raise ValueError("Incomplete acquired/denied source inventory")
    return manifest
